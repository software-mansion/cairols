...
#
# Input (stdin): JSON array of rows produced by notify_pending_reviews.sh, one
# entry per (reviewer, PR) pair, with shape:
#   { repo, reviewer, title, url, number, waiting_s, additions, deletions }
#
...
# Short "repo#123" label from the full owner/name repo slug.
def short_ref:
  "\(.repo | split("/") | last)#\(.number)";

# Split a string on backticks and emit one rich_text text element per segment;
# odd-indexed segments are rendered as inline code.
def title_elements:
  split("`")
  | to_entries
  | map(select(.value != ""))
...
if length == 0 then
  {
    blocks: [
      ...
      {type: "section", text: {type: "mrkdwn",
        text: ":tada: No pending review requests across the tracked repos."}}
    ]
  }
else
  group_by(.reviewer) as $groups
  | ([.[].repo] | unique | length) as $repo_count
  | {
      blocks: (
        [
          header_block,
          {type: "context", elements: [
            {type: "mrkdwn", text: "\(plural(length; "pending review")) across \(plural($groups | length; "reviewer")) and \(plural($repo_count; "repo"))"}
          ]}
        ]
        + ($groups | map(
            . as $group
            | ($group | map(.waiting_s | humanize | length) | max) as $wmax
            | ($group | map(short_ref | length) | max) as $nmax
            | [
                {type: "divider"},
                {type: "section", text: {type: "mrkdwn",
                  ...
                  elements: ($group | map(
                    ...
                    | {
                        ...
                        elements: ([
                          ...
                          {type: "text",  text: "  "},
                          {type: "link",  url: .url, text: rpad($ref; $nmax)},
                          {type: "text",  text: "  "}
