...
fn doc_link_segments_for_offset<'db>(
    ...
    if std::env::var("CAIRO_LS_DEBUG_DOC_LINK").is_ok() {
        eprintln!(
            "doc-link cursor offsets: cursor_offset={}, dest_span_start={}, dest_span_end={}, relative_cursor_offset={}, dest_text_len={}",
            cursor_offset.as_u32(),
            dest_span.start.as_u32(),
            dest_span.end.as_u32(),
            relative_cursor_offset,
            dest_text.len()
        );
        let cursor_byte = dest_text.as_bytes().get(relative_cursor_offset as usize).copied();
        eprintln!("doc-link cursor byte: {:?}, dest_text={:?}", cursor_byte, dest_text);
    }
