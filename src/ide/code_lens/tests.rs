...
fn collect_test_lenses<'db>(
    ...
    for node in collect_test_functions(db, module) {
        let is_fuzzer = is_fuzzer_test(db, node.attribute_ptr);
...
fn is_fuzzer_test(db: &AnalysisDatabase, ptr: SyntaxStablePtrId) -> bool {
    ...
    // If the originating location is at a #[test_case] attribute, this is a concrete test case
    // generated by the test_case macro — not a fuzzer. The original code used
    // `ancestor_of_type::<Attribute>` (which excludes self) for this check, but
    // `find_syntax_node_at_offset` can return the attribute node itself, so we use
    // `ancestors_with_self` to include self in the search.
    ...
    // Remaining case: either a pure #[fuzzer] or the fuzzer entry of a #[fuzzer]+#[test_case]
    // function. Both have #[fuzzer] on the originating source function.
    //
    original_node
