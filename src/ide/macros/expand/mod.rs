...
use expr::expand_inline_macros;
use format::format_output;
use inlining::{apply_changes, new_span};
use module_item::expanded_macros;
...
fn expand_macro_ex(
    ...
    let file_with_inlined_module_items = if item_node.kind(db) == SyntaxKind::ExprInlineMacro {
        ...
        let mut files = vec![];
        expanded_macros(db, module_item, crate_id, metadata, &mut files)?;

        // Inline all files that should be inlined, keep others in `extra_files` for further processing.
        let replaced_content = apply_changes(db, file_to_process, &files, &mut extra_files)?;
    ...
    let extra_files = extra_files
        ...
        .filter_map(|content| {
            ...
            let expanded_items = db
                .file_module_syntax(file)
                .ok()?
                .items(db)
                .elements(db)
                .into_iter()
                .map(|item| {
                    expand_macro_ex(db, file, crate_id, metadata, item.as_syntax_node()).unwrap()
                })
                .collect::<Vec<_>>();
    ...
    let span = new_span(db, file_to_process, file_with_inlined_module_items, item_node.span(db))?;

    let replaced_content_file =
        expand_inline_macros(db, crate_id, file_with_inlined_module_items, span, metadata)?;

    // Span of replaced content in new generated file.
    let new_span = new_span(db, file_to_process, replaced_content_file, item_node.span(db))?;
