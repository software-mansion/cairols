use super::inlining::{ExpandMacroInliningStrategy, FileWithOrigin, apply_changes, new_span};
...
pub fn expand_inline_macros(
    ...
    for inline_macro in inline_macros {
        ...
        files.push(FileWithOrigin {
            file,
            generated_from: inline_macro.as_syntax_node(),
            strategy: ExpandMacroInliningStrategy::Replace,
        });
    }
    ...
    let replaced_content = apply_changes(db, file_to_process, &files, &mut Default::default())?;
    ...
    .intern(db);
    let span = new_span(db, file_to_process, new_file, expand_in)?;

    // If we resolved anything, try recursive call to resolve any potential untouched macro call.
    expand_inline_macros(db, crate_id, new_file, span, metadata)
}
