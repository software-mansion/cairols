...
use cairo_lang_syntax::node::ast::{ItemModule, PathSegment, SkippedNode, TriviumSkippedNode};
...
pub fn top_level_inline_macro_completions<'db>(
    ...
    let is_descendant_of_module_item = ctx
        .node
        .ancestor_of_kinds(
            db,
            &[
        ...
        path.segments(db).elements(db).take(2).collect_vec().as_slice()
    {
        ...
        let compile_error_plugin_present = macro_plugins.iter().any(|id| {
            id.long(db).plugin_type_id() == CompileErrorPlugin::default().plugin_type_id()
        });

        let available_top_level_inline_macros = chain!(
