...
use cairo_lang_syntax::node::{
    ...
    ast::{PatternStruct, PatternStructParam, TerminalIdentifier},
};
...
pub fn struct_pattern_completions(
    ...
    let (all_members, existing_members, typed) = if_chain!(
        ...
        if let typed = ctx.node.ancestor_of_type::<TerminalIdentifier>(db).filter(|ident| {
    ...
    let typed = typed.map(|ident| ident.token(db).text(db)).unwrap_or_default();
