...
use super::{
    ManifestActionContext, extra_diagnostic_data_as_string_array, replace_manifest_action,
};

pub fn build(ctx: &ManifestActionContext<'_>) -> Vec<CodeAction> {
    let Some(inherits_path) = extra_diagnostic_data_as_string_array(ctx.diagnostic, "field_path")
    else {
        return Vec::new();
    };
    let Some(valid_profiles) =
        extra_diagnostic_data_as_string_array(ctx.diagnostic, "valid_values")
    else {
        return Vec::new();
    };
