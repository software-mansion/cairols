...
use super::{
    ManifestActionContext, extra_diagnostic_data_as_string_array, remove_key_path,
    replace_manifest_action,
};

pub fn build(ctx: &ManifestActionContext<'_>) -> Vec<CodeAction> {
    let Some(inlining_strategy_path) =
        extra_diagnostic_data_as_string_array(ctx.diagnostic, "inlining_strategy_path")
    else {
        return Vec::new();
    };
    let Some(skip_optimizations_path) =
        extra_diagnostic_data_as_string_array(ctx.diagnostic, "skip_optimizations_path")
    else {
        return Vec::new();
    };
