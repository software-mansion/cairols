...
pub fn suggest_similar_member<'db>(
    ...
    let expr = semantic_db.expr_semantic(function_with_body, expr_id);
    ...
    let typed_member_name = ctx.node.get_text(db);
    ...
    let suggestions: Vec<String> = member_candidates
        ...
        .filter_map(|name_ref| {
            ...
            if text_matches(name, typed_member_name) { Some(name.to_string()) } else { None }
        })
        .collect();
