    ...
    db.module_uses_ids(ctx.module_id)
        ...
        .and_then(|uses| {
            ...
            uses.iter()
                ...
                .map(|stabel_ptr| UsePosition::new(db, stabel_ptr, true))
                .map(Some)
                .unwrap_or_else(|| {
...
impl UsePosition {
    pub fn new(db: &AnalysisDatabase, ptr: SyntaxStablePtrId, is_sticking: bool) -> Self {
        ...
        Self {
            position: node
                ...
                .position_in_file(
                    db,
                    db.module_file(db.find_module_file_containing_node(&node).unwrap()).unwrap(),
                )
                .unwrap()
                .to_lsp(),
            is_sticking,
