...
impl<'db> SymbolSearch<'db> {
    ...
    fn from_resolved_item(
        ...
        match resolved_item {
            ...
            // TODO: Instead of a single `GenericParamDef`, define a struct for each kind of generic param (type, const, named impl),
            // match the variants of `GenericParam` here and build those structs.
            // There's no need to create the enum for those structs because each type of generic param requires a dedicated hover logic.
