...
pub struct GenericParamDef<'db> {
    ...
    details: GenericParamDetails<'db>,
}

/// Details of the generic parameter, corresponding to its kind.
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum GenericParamDetails<'db> {
    Type,
    Const { ty: TypeId<'db> },
...
impl<'db> GenericParamDef<'db> {
    ...
    pub(super) fn new(
        ...
        let details = match generic_param {
            GenericParam::Type(_generic_param_type) => GenericParamDetails::Type,
            GenericParam::Const(generic_param_const) => {
                GenericParamDetails::Const { ty: generic_param_const.ty }
            }
            GenericParam::Impl(generic_param_impl) => {
                GenericParamDetails::Impl { concrete_trait: generic_param_impl.concrete_trait.ok() }
            }
    ...
    pub fn signature(&self, db: &AnalysisDatabase) -> String {
        ...
        match self.details {
            GenericParamDetails::Type => self.name(db).to_string(),
            GenericParamDetails::Const { ty } => {
                let formatted_type = format_type(db, ty, &importables());
                format!("const {}: {}", self.name(db), formatted_type)
            }
            GenericParamDetails::Impl { concrete_trait } => {
