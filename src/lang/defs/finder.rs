...
pub enum ResolvedItem<'db> {
    ...
    ExprDeclarativeMacro(MacroDeclarationId<'db>),
        ...
        && macro_name.identifier(db) == identifier.text(db)
    {
        ...
        match resolver.resolve_generic_path(
            ...
            Ok(ResolvedGenericItem::Macro(macro_declaration_id)) => {
                Some(ResolvedItem::ExprDeclarativeMacro(macro_declaration_id))
            }
...
impl<'db> ResolvedItem<'db> {
    ...
    pub fn definition_stable_ptr(
        ...
        let stable_ptr = match self {
            ...
            ResolvedItem::ExprDeclarativeMacro(declaration_id) => {
                declaration_id.stable_ptr(db).untyped()
            }
        };
        Some(stable_ptr)
    }
