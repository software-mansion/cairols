...
use cairo_lang_defs::db::{
    ...
    init_defs_group, init_external_files, register_inline_macro_plugin_override_view,
...
use cairo_lang_filesystem::db::{
    CrateConfig, CrateConfigStorage, CrateConfigView, CrateConfigurationInput, FileContentOverride,
    FileContentStorage, FileContentView, FilesGroup, files_group_input, init_files_group,
...
use cairo_lang_semantic::db::{
    ...
    PluginSuiteInput, init_semantic_group, register_analyzer_plugin_override_view,
    semantic_group_input,
};
...
pub struct AnalysisDatabase {
    ...
    current_crate_plugin_suites: Arc<RwLock<OrderedHashMap<CrateInput, CratePluginSuiteInputs>>>,
    current_crate_plugin_suite_fingerprints:
...
impl CratePluginSuiteFingerprint {
    fn from_plugin_suite(value: &PluginSuite) -> Self {
        Self {
            macro_plugins: Arc::from(value.plugins.iter().map(plugin_identity).collect::<Vec<_>>()),
            analyzer_plugins: Arc::from(
                value.analyzer_plugins.iter().map(plugin_identity).collect::<Vec<_>>(),
            ),
            inline_macro_plugins: Arc::new(
                value
                    .inline_macro_plugins
...
impl AnalysisDatabase {
    ...
    pub fn new() -> Self {
        let mut db = Self {
            ...
            current_crate_plugin_suites: Default::default(),
            current_crate_plugin_suite_fingerprints: Default::default(),
    ...
    fn file_contents_handle(&self, file_input: &FileInput) -> Option<FileContentOverride> {
...
pub fn ensure_crate_config_handle_for_input(&mut self, crate_input: CrateInput) -> CrateConfig {
    ...
    fn ensure_file_contents_handle_for_input(&mut self, file_input: FileInput) -> FileContentOverride {
        if let Some(handle) = self.file_contents_handle(&file_input) {
            return handle;
        }

        let handle = FileContentOverride::new(self, None);
    ...
    pub fn clear_generated_file_contents(&mut self) {
        ...
        for handle in handles {
            handle.set_content(self).with_durability(Durability::HIGH).to(None);
        }
    }
    ...
    fn ensure_macro_plugin_override_handle(
        ...
        let next = defs_group_input(self).macro_plugin_overrides_revision(self).saturating_add(1);
    ...
    fn ensure_inline_macro_plugin_override_handle(
        ...
        let next =
            defs_group_input(self).inline_macro_plugin_overrides_revision(self).saturating_add(1);
        defs_group_input(self)
    ...
    fn ensure_analyzer_plugin_override_handle(
        ...
        let next =
            semantic_group_input(self).analyzer_plugin_overrides_revision(self).saturating_add(1);
    ...
    pub fn set_override_crate_plugins_from_suites(
        ...
        for (crate_input, plugins) in suites {
            ...
            if self.current_crate_plugin_suite_fingerprints.read().unwrap().get(&crate_input)
                == Some(&fingerprint)
            {
                continue;
            }
            self.current_crate_plugin_suites
                .write()
                .unwrap()
                .insert(crate_input.clone(), suite_inputs.clone());
            self.current_crate_plugin_suite_fingerprints
                .write()
                .unwrap()
    ...
    pub fn clear_override_crate_plugins(&mut self, crate_input: CrateInput) {
        self.current_crate_plugin_suites.write().unwrap().swap_remove(&crate_input);
        self.current_crate_plugin_suite_fingerprints.write().unwrap().swap_remove(&crate_input);
        self.apply_crate_plugin_suite_for_input(crate_input, None);
    }

    pub fn sync_crate_plugin_suites(
        &mut self,
        suites: impl IntoIterator<Item = (CrateInput, PluginSuite)>,
    ) {
        let (changed, removed) = {
            let mut current_fingerprints =
                self.current_crate_plugin_suite_fingerprints.write().unwrap();
            let mut current = self.current_crate_plugin_suites.write().unwrap();
    ...
    pub fn remove_crate_plugin_suites(
        ...
        for (crate_input, plugins) in suites {
            let Some(current) =
                self.current_crate_plugin_suites.read().unwrap().get(&crate_input).cloned()
            else {
                continue;
            };
            ...
            self.current_crate_plugin_suite_fingerprints
                .write()
                .unwrap()
                .insert(crate_input.clone(), CratePluginSuiteFingerprint::from_inputs(&next_suite));
    ...
    pub fn add_proc_macro_plugin_suite(&mut self, crate_input: CrateInput, plugins: PluginSuite) {
        ...
        self.current_crate_plugin_suite_fingerprints
            .write()
            .unwrap()
            .insert(crate_input.clone(), CratePluginSuiteFingerprint::from_inputs(&current));
