...
impl AnalysisDatabase {
    ...
    /// Updates the plugin list in the database for each crate.
    ///
    ...
    /// considered identical based on their Salsa short ID equality (`Arc::ptr_eq` under the hood).
    pub fn replace_crate_plugin_suite(
        &mut self,
        crate_id: CrateId,
        plugins_to_remove: &InternedPluginSuite,
        plugins_to_add: InternedPluginSuite,
    ) {
    ...
    fn with_plugins_mut(
        ...
        self.set_override_crate_macro_plugins(
            crate_id,
            macro_plugins.into_iter().collect::<Arc<[_]>>(),
        );
        self.set_override_crate_analyzer_plugins(
            crate_id,
            analyzer_plugins.into_iter().collect::<Arc<[_]>>(),
        );
