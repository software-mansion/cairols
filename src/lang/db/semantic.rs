...
use cairo_lang_filesystem::db::{get_external_files, get_parent_and_mapping, translate_location};
    ...
    for file in node_descendant_files.iter().cloned() {
        ...
        let is_replacing_og_item = match file.long(db) {
            ...
            FileLongId::External(id) => {
                get_external_files(db).ext_as_virtual(db, *id).original_item_removed
            }
            _ => unreachable!(),
        };
