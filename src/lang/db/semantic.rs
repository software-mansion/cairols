...
fn find_module_containing_node<'db>(
    ...
    let main_module = {
        ...
        let mut node_file_id = node.stable_ptr(db).file_id(db);

        // db.file_modules() is DefsGroup defined so for every inline macro expression file there will be no entry
        // go up to first non-expression file.
        while db.file_modules(node_file_id).is_err() {
            node_file_id = match node_file_id.long(db) {
                FileLongId::OnDisk(_) => return None,
                FileLongId::External(ext) => ext_as_virtual(db, *ext).parent?,
                FileLongId::Virtual(vfs) => vfs.parent?,
            }
            .file_id;
        }
...
fn file_and_subfiles_with_corresponding_modules<'db>(
    db: &'db dyn Database,
    file: FileId<'db>,
) -> (OrderedHashSet<FileId<'db>>, OrderedHashSet<ModuleId<'db>>) {
    let file_modules = db.file_modules(file).expect("file must be OnDisk");
    ...
    while let Some(module_id) = modules_queue.pop_front() {
        ...
        for &inline_macro_file in inline_macro_files {
            ...
            while let Some((parent, _)) = get_parent_and_mapping(db, inline_macro_file) {
                ...
                if files.contains(&inline_macro_file) {
...
fn find_generated_nodes<'db>(
    ...
    for file in node_descendant_files.iter().cloned() {
        ...
        for mapping in &mappings {
            ...
            node_parent.unwrap_or(node_by_offset).for_each_terminal(db, |terminal| {
