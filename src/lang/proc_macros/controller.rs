...
impl ProcMacroClientController {
    ...
    pub fn handle_response(
        ...
        // Safety: the client is moved to `apply_responses`.
        // No other references to it should exist at this point.
    ...
    pub fn handle_config_update(&mut self, db: &mut AnalysisDatabase, config: &Config) {
        if !config.enable_proc_macros {
            ...
            self.remove_all_proc_macros_from_db(db);
        ...
        if db.proc_macro_input().proc_macro_server_status(db).is_pending() {
            trace!("[PMS] Restarting due to config change");
    ...
    /// Requests the proc-macro-server to load the workspace with the given `manifest_path`.
    pub fn discover_workspace(&self, db: &AnalysisDatabase, manifest_path: PathBuf) {
        if let ServerStatus::Connected(client) = db.proc_macro_input().proc_macro_server_status(db)
        {
            trace!(
                "[ProcMacroController] Sending discoverWorkspace request for {}",
                manifest_path.to_string_lossy().to_string()
            );
            client.request_discover_workspace(DiscoverWorkspaceParams {
    ...
    fn apply_responses(
        ...
        let mut workspace_for_defined_macros = vec![];
        ...
        for (params, response) in available_responses {
            match params {
                RequestParams::DiscoverWorkspace(params) => {
                    let _response = parse_response::<DiscoverWorkspaceResponse>(response)?;
                    workspace_for_defined_macros.push(params.workspace);
                }
        ...
        for workspace in workspace_for_defined_macros {
            trace!("[ProcMacroController] Requesting definedMacros for workspace {:?}", workspace);
            client.request_defined_macros(DefinedMacrosParams { workspace });
        }
    ...
    /// Launches proc-macro-server if it is allowed by the config.
