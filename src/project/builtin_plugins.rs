use std::{any::TypeId, collections::HashMap, iter::repeat, sync::LazyLock};
...
static PLUGIN_TYPE_IDS: LazyLock<HashMap<TypeId, BuiltinPlugin>> = LazyLock::new(|| {
    chain!(
        plugin_type_ids(test_assert_suite()).zip(repeat(BuiltinPlugin::AssertMacros)),
        plugin_type_ids(test_plugin_suite()).zip(repeat(BuiltinPlugin::CairoTest)),
        plugin_type_ids(cairo_run_plugin_suite()).zip(repeat(BuiltinPlugin::CairoRun)),
        plugin_type_ids(executable_plugin_suite()).zip(repeat(BuiltinPlugin::Executable)),
        plugin_type_ids(starknet_plugin_suite()).zip(repeat(BuiltinPlugin::Starknet)),
    )
    .collect()
});
